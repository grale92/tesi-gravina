...
In primo luogo l'inizializzazione del modello avviene con la funzione \\ \texttt{spacy.blank()}, che come \textit{side effect} scatena la generazione di un file \texttt{config.cfg} contenente tutte le informazioni legate al modello.
...
Al termine dello script i due dataset vengono salvati pronti per essere utilizzati nella fase di training, prima della quale è possibile fare eventuali regolazioni del modello e dei suoi iperparametri. La pipeline scelta è composta da due soli componenti: \texttt{tok2vec} e \texttt{ner}.

La componente \texttt{tok2vec} ha il ruolo di costruire a partire dai token una rappresentazione vettoriale densa per catturare le proprietà contestuali utili per la fase successiva, in breve è la componente che costituisce la parte di \textit{feature learning} del sistema. La componente \texttt{ner} utilizza tali rappresentazioni vettoriali per risolvere il problema di \textit{sequence labeling}, stimando i confini delle entità e assegnando a ciascuna una delle etichette previste.

La fase di training viene eseguita direttamente da terminale all'interno del container andando a specificare i \textit{path} contenenti i dataset ed a fornire il file di configurazione del modello.
\\
\begin{lstlisting}
...
\begin{enumerate}
    \item \textbf{Acquisizione del documento}: La classe \texttt{UploadFile} consente di gestire il file come \textit{multipart/form-data} e riceverne il contenuto.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/interfaccia_start.png}
    \caption{Interfaccia per il caricamento dei report}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/interfaccia_end.png}
    \caption{Esempio di interfaccia dopo l'elaborazione del report}
\end{figure}

\subsection{Recupero dei dati aziendali e strategie di matching "debole"}

Le entità estratte dal modello NER non coincidono necessariamente in modo perfetto con la rappresentazione presente nel database aziendale, nonostante una fase di normalizzazione già presente in fase di elaborazione.

Di conseguenza, per questa prima fase di integrazione è stata utilizzata una strategia che non si basi esclusivamente su confronti esatti. L'associazione è stata implementata come un matching rilassato, per tollerare differenze sintattiche e piccole variazioni testuali. Le tre tecniche utilizzate sono principalmente:

\begin{itemize}
    \item \textbf{Normalizzazione preventiva delle stringhe}. Le entità estratte subiscono un'ulteriore fase di normalizzazione, che consiste in operazioni quali la conversione in minuscolo, rimozione di spazi superflui, uniformazione di simboli e separatori, eliminazione di caratteri non significativi.
    \item \textbf{Matching parziale su campi strutturati}. Per campi altamente discriminanti quali il codice articolo, l'anno stagione o numeri di standard di certificazione, viene effettuato un confronto parziale, privilegiando la presenza di pattern all'interno delle stringhe.
    \item \textbf{Fuzzy matching su campi descrittivi}. Per le entità di tipo descrittivo, come il nome del test o il metodo normativo, è stato adottato un confronto basato su misure di similarità lessicale. In particolare viene calcolato un punteggio di similarità utilizzando metriche basate su distanza edit e similarità dei token.
\end{itemize}

Queste tecniche, tuttavia, all'aumentare delle varianti documentali, possono comunque portare a falsi negativi, richiedendo maggior intervento manuale sulla fase di validazione. Per superare tali limitazioni, un'evoluzione naturale prevederebbe l'introduzione di tecniche basate su rappresentazioni semantiche, come la trasformazione delle descrizioni testuali in vettori numerici (\textit{embeddings}) e il confronto nello spazio vettoriale tramite misure di similarità.