...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/pdf_stream.png}
  \caption{Esempio di uno stream PDF}
\end{figure}
...
I sistemi di Named Entity Recognition possono essere classificati in base alla metodologia adottata per l'identificazione delle entità. Di seguito verranno elencati gli approcci più comuni alla problematica.
...
Nell'ambito del machine learning, il NER viene comunemente affrontato come un problema di \textit{sequence labeling}, in cui a ciascun token che compone una sequenza testuale viene assegnata un'etichetta appartenente a un insieme predefinito di classi \cite{jm3}.

L'introduzione delle tecniche di apprendimento automatico hanno consentito il superamento dei limiti delle tecniche basate su regole manuali e dizionari, attraverso modelli statistici in grado di rilevare automaticamente le caratteristiche discriminanti delle entità a partire da testi annotati. Tra i primi esempi di modelli utilizzati troviamo gli \textit{Hidden Markov Models (HMM)} e i \textit{Conditional Random Fields (CRF)}.

Gli Hidden Markov Models appartengono alla classe dei modelli generativi, in quanto definiscono una distribuzione di probabilità congiunta sulle sequenze di osservazioni e sulle corrispondenti etichette \cite{rabiner}. Come evidenziato nella letteratura, tali modelli presentano limiti intrinseci nell'individuazione di caratteristiche complesse e dipendenze a lungo raggio tra le osservazioni, rendendo spesso l'inferenza onerosa a livello computazionale o addirittura ingestibile. Inoltre, l'inclusione di un elevato numero di entità risulta difficilmente gestibile all'interno di un framework generativo \cite{lafferty}.

Per superare queste limitazioni, sono stati introdotti modelli discriminativi per il labeling sequenziale, tra cui i \textit{Maximum Entropy Markov Models (MEMM)}. Sebbene questi modelli consentano di includere un insieme più ricco di entità e di considerare il contesto locale in fase di predizione, essi soffrono del problema detto \textit{label bias problem}, dovuto alla normalizzazione locale delle probabilità di transizione, che può penalizzare la corretta definizione delle dipendenze all'interno di una sequenza.

I Conditional Random Fields rappresentano un'evoluzione significativa in quest'ambito, combinando i vantaggi dei modelli generativi e di quelli discriminativi. Un CRF è un modello probabilistico discriminativo per il labeling sequenziale che stima direttamente la probabilità condizionata di una sequenza di etichette data una sequenza di osservazioni. A livello formale, dato un grafo:
\[
G = (V, E)
\]
e una collezione di variabili casuali:
\[
Y = \{ Y_v \}_{v \in V}
\]
in modo che $Y$ sia indicizzato dai vertici di $G$. Allora la coppia $(X, Y)$ definisce un Conditional Random Field se, condizionatamente a $X$, le variabili $Y_v$ soddisfano la proprietà di Markov rispetto al grafo, ovvero se ogni variabile dipende unicamente dalle variabili associate ai nodi adiacenti.

L'addestramento di un CRF avviene massimizzando il logaritmo della funzione di verosimiglianza condizionata dalle etichette osservate rispetto ai dati di input, permettendo al modello di considerare simultaneamente il contesto passato e futuro durante il processo di apprendimento. Questa caratteristica rende i CRF particolarmente adatti a compiti di Named Entity Recognition, nei quali l'etichetta assegnata a un token dipende fortemente dalle informazioni contestuali e dalle altre etichette circostanti.

L'impiego di tali modelli ha rappresentato a lungo lo stato dell'arte nei sistemi NER basati su apprendimento supervisionato, rappresentando una base solida per lo sviluppo di soluzioni più avanzate basate su reti neurali complesse e su architetture \textit{transformer}.

\subsection{NER basato su Reti Neurali e Transformer}

L'uso di reti neurali ha in primo luogo consentito di ridurre la dipendenza della progettazione manuale delle entità, affidando al modello stesso il compito di estrarre le informazioni rilevanti per il compito di Named Entity Recognition.

Tra i primi approcci di reti neurali applicati alla NER è possibile individuare le \textit{Convolutional Neural Networks (CNN)}, in origine sviluppate per l'elaborazione di dati strutturati come immagini e successivamente adattate al dominio testuale. Le CNN vengono applicate per cattutare pattern locali all'interno delle sequenze di token, attraverso l'applicazione di filtri convoluzionali su finestre di parole adiacenti. Questo metodo risulta efficace nell'individuare sottosequenze rilevanti e strutture locali ricorrenti, portando a un miglioramento della qualità delle rappresentazioni testuali utilizzate per classificare le entità \cite{collobert}.

Tuttavia, le CNN presentano delle limitazioni sulla modellazione di dipendenze a lungo raggio, poiché la loro capacità di integrare informazioni distanti all'interno di una sequenza dipende dalla profondità della rete e dalla dimensione delle finestre convoluzionali. Per superare tale limite, la tendenza è stata passare a modelli in grado di rappresentare il contesto globale della sequenza in modo più efficace.

Un'evoluzione in tal senso è rappresentata dall'introduzione delle architetture \textit{transformer}, che a differenza delle reti convoluzionali, consentono di modellare direttamente le relazioni tra tutti i token di una sequenza, indipendentemente dalla loro distanza, permettendo al modello di definire al meglio le dipendenze globali legate al contesto \cite{vaswani}.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.6\textwidth]{figures/transformer_arch.png}
  \caption{Architettura di un transformer}
  \label{fig:transformer_arch}
\end{figure}

I concetti fondamentali di un'architettura transformer, come quella mostrata d'esempio in Figura~\ref{fig:transformer_arch}, possono essere suddivisi nei punti elencati di seguito.

\textbf{Meccanismo di Self-Attention}
    
Questo meccanismo è formalizzato anche come \textit{scaled dot-product attention}. Data una matrice di input \( Z \in \mathbb{R}^{n \times d_{\text{model}}} \), vengono calcolate tre matrici $Q$, $K$ e $V$, denominate rispettivamente \textit{query}, \textit{key} e \textit{value}, mediante proiezioni lineari apprese. Il meccanismo di attenzione è definito come:
...
dove $d_k$ è la dimensione delle key e $M$ rappresenta una maschera opzionale per limitare le dipendenze ammesse. Questa funzione determina quanto sono rilevanti nel contesto gli altri token della sequenza.

\textbf{Multi-Head Attention}

Il meccanismo di attenzione appena descritto viene replicato su più sottospazi indipendenti, al fine di aumentare la capacità espressiva del modello. Le diverse repliche permettono al modello di catturare simultaneamente diverse tipologie di relazioni, che poi vengono concatenate e reinserite nello spazio $d_{model}$.

\textbf{Feed-Forward Network}

La rete neurale \textit{feed-forward} viene applicata indipendentemente a ciascuna posizione della sequenza. Entrambi i sottoblocchi sono circondati da connessioni residue e normalizzazioni di strato (\textit{Layer Normalization}), che migliorano la stabilità numerica e facilitano l'addestramento di architetture profonde. Tipicamente la rete è formata da due trasformazioni lineari con attivazione \textit{ReLu}.

\textbf{Embeddings}

I transformer non possono lavorare con il testo grezzo ma necessitano di numeri. Pertanto, ogni token di input viene convertito in un vettore, chiamato \textit{embedding}. Questi embedding sono addestrabili, ovvero che il modello impara la miglior rappresentazione numerica per ogni token. Poiché il meccanismo di attenzione è invariante rispetto alle permutazioni, l'informazione relativa all'ordine della sequenza viene incorporata attraverso \textit{positional encoding}, che viene sommato agli embedding dei token. Nel modello originale tali codifiche sono deterministiche e basate su funzioni sinusoidali, permettendo al modello di generalizzare a sequenze di lunghezza non osservata in fase di addestramento.

\textbf{Architettura Encoder-Decoder}

La struttura \textit{encoder-decoder} è la chiave dei modelli transformer. L'\textit{encoder} processa la sequenza di input in un vettore, mentre il \textit{decoder} ri-trasforma questo vettore in una nuova sequenza. Ogni encoder e decoder includono meccanismi di self-attention e livelli feed-forward. Dopo che il decoder ha processato la sequenza, deve predirre il token successivo. L'output del decoder viene passato a un \textit{linear layer} i cui pesi sono condivisi con gli embedding. Infine, la funzione \textit{softmax} converte i punteggi ottenuti in probabilità, da cui si ottiene il token con la probabilità più alta che diventa la successiva parola predetta.
