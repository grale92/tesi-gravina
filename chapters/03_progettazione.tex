...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/flusso_completo.png}
    \caption{Rappresentazione del flusso applicativo}
    \label{fig:flusso_completo}
\end{figure}
...
\textbf{Caricamento del report}.   L'utente dell'ente di certificazione accede a una pagina dedicata all'interno dell'applicativo, affiancata a quelle già esistenti e distinta da esse per la semplicità dell'interfaccia. All'apertura della pagina infatti, l'utente visualizza esclusivamente un modulo di caricamento che consente di effettuare l'upload del documento PDF, o tramite trascinamento o tramite ricerca nelle cartelle del dispositivo utilizzato. Il documento racchiude i risultati dei test di laboratorio eseguiti su uno specifico articolo o modello.
...
\textbf{Scalabilità}. Il sistema dev'essere progettato in modo da supportare un numero crescente di documenti senza degradare significativamente le prestazioni e mantenendo dei tempi di risposta che siano compatibili con quelli di un utilizzo interattivo dell'interfaccia web, in modo da non compromettere l'esperienza utente.
...
La fase di addestramento (\textit{training}) è un processo supervisionato, nel quale il modello viene ottimizzato a partire dai dati annotati manualmente tramite uno strumento di labeling. In particolare, le annotazioni vengono esportate e successivamente convertite in un formato nativo compatibile con la libreria scelta, necessario per l’avvio della fase di training.
...
Il processo di training si basa sull'obiettivo di minimizzare una funzione di perdita che misura la differenza tra le etichette predette dal modello e quelle annotate manualmente. Il processo avviene in più iterazioni, in ciascuna delle quali il modello produce una predizione per ogni token della sequenza e aggiorna i propri parametri interni in modo da ridurre progressivamente l'errore complessivo. Questo meccanismo consente al sistema di apprendere in modo sempre più conforme al dominio applicativo specifico.
...
Un ulteriore aspetto rilevante nella progettazione della fase di addestramento consiste nella configurazione degli iperparametri del modello. Questi vengono definiti prima dell'avvio del processo e influenzano in modo significativo il comportamento del modello.

Tra gli iperparametri principali rientrano:

\begin{itemize}
    \item il \textbf{numero di epoche di addestramento}, ovvero i cicli di addestramento attraverso l'intero set di dati;
    \item la \textbf{dimensione del batch}, ovvero il numero di esempi utilizzati per ciascun aggiornamento dei parametri;
    \item il \textbf{learning rate}, ovvero l'ampiezza degli aggiornamenti applicati ai parametri del modello.
\end{itemize}

La scelta di tali iperparametri incide direttamente sull'equilibrio tra capacità di apprendimento e generalizzazione. Un batch di dimensione ridotta introduce maggiore variabilità negli aggiornamenti, mentre batch più grandi rendono l'ottimizzazione stabile ma richiedono più risorse computazionali. Un learning rate troppo elevato può causare instabilità e oscillazioni nella funzione di perdita, al contrario, un valore troppo basso può rallentare eccessivamente il miglioramento delle predizioni del modello.

Durante l'addestramento è fondamentale monitorare l'evoluzione delle metriche sul dataseset di validazione, al fine di prevenire fenomeni di \textit{overfitting}. Per fare ciò vengono utilizzate strategie di \textit{early stopping}, interrompendo l'addestramento se le prestazioni sui dati di valutazione non mostrano ulteriori miglioramenti. Infine, il modello con le migliori prestazioni viene salvato come checkpoint finale per l'utilizzo in fase di inferenza.

\subsection{Utilizzo del modello NER}

Una volta completata la fase di addestramento, il modello NER viene integrato all'interno dell'architettura applicativa come componente dedicato all'estrazione automatica delle entità dai documenti PDF caricati dall'utente.

Dal punto di vista progettuale, il modello viene incapsulato all'interno di un servizio applicativo indipendente, che viene richiamato dal frontend. Tale scelta è stata fatta almeno in questa fase sperimentale per separare la logica di inferenza dal resto dell'applicazione, garantendo modularità e la possibilità in futuro di sostituire il modello senza impattare sul resto del sistema.

A livello operativo il flusso previsto è il seguente:

\begin{enumerate}
    \item L'utente carica un documento PDF tramite l'interfaccia web.
    \item Il documento viene sottoposto a una fase preliminare simile a quella subita dal dataset di training, che prevede l'estrazione del testo e la segmentazione in finestre testuali.
    \item L'oggetto risultante viene fornito in input al modello NER.
    \item Il modello produce un insieme di entità riconosciute, ciascuna associata a un'etichetta.
    \item Le entità estratte vengono normalizzate e trasformate in una struttura dati coerente con lo schema previsto dall'applicazione.
\end{enumerate}

Un aspetto centrale riguarda la standardizzazione dell'output, poiché le entità riconosciute dal modello costituiscono l'input per una fase di matching verso i dati aziendali. In questa fase le informazioni estratte vengono confrontate con i record presenti nel sistema informativo, per individuare le prove corrispondenti al documento analizzato.

Tale corrispondenza non può basarsi su uguaglianze esatte, poiché i documenti provenienti da enti diversi possono presentare variazioni terminologiche o differenze di formattazione. Per questo motivo è stato previsto, almeno per questa fase sperimentale, l'utilizzo di confronti parziali, tecniche di fuzzy matching e normalizzazione preventiva delle stringhe.
...
L’integrazione con un servizio cloud-based, così come per la soluzione custom vista in precedenza, viene progettata come un componente indipendente all’interno dell’architettura applicativa, accessibile tramite interfacce REST e sostituibile senza impatti sulle altre componenti del sistema. In questo modo, il servizio di estrazione delle informazioni può essere visto come una "black box" che riceve in input un documento PDF e restituisce un output strutturato secondo uno schema predefinito, coerente con le esigenze dell’applicazione. Oltre a questo, anche questo servizio si occuperà della fase finale di pulizia e normalizzazione dei dati restituiti dal servizio esterno, in modo da standardizzare l'output rendendolo coerente con quello definito tramite implementazione custom.
...
\label{sec:associazione-documento-entita}

Per evidenziare al meglio le peculiarità del processo di estrazione automatica delle informazioni, è necessario fare una panoramica sulla selezione delle entità effettuata. I documenti di input contengono una grande quantità di informazioni eterogenee e di rumore, tra cui dati anagrafici del laboratorio, riferimenti normativi, descrizioni testuali e fotografiche del campione, dati amministrativi e risultati delle prove. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{figures/report_facsimile.png}
    \caption{Fac-simile di un report di laboratorio}
\end{figure}

Tuttavia, come emerso in fase di analisi dei requisiti, lo scopo finale è quello di permettere all'utente di validare in modo rapido i test effettuati su uno specifico articolo o modello finito, pertanto i dati rilevanti da raccogliere sono una piccola parte e nel raggiungere l'obiettivo designato è stato necessario operare una riduzione mirata dello spazio informativo, individuando un insieme ristretto di entità considerate rilevanti e in comune tra i diversi template documentali.

In particolare, di seguito verranno analizzate, senza scendere in dettagli legati alla base dati, le categorie di dati da estrarre e le loro relazioni:

\begin{itemize}
    \item \textbf{Identificativo del campione} : può essere il codice dell'articolo o di un modello finito, costituisce l'entità che fornisce il collegamento diretto al database aziendale
    \item \textbf{Dati descrittivi del campione} : sono dati di supporto all'identificativo che permettono di inquadrare meglio il campione, come l'anno stagione della collezione, il brand (una società di abbigliamento in genere gestisce più brand), il colore o la variante a seconda del tipo di campione.
    \item \textbf{Informazioni associate al report} : principalmente il numero identificativo del report e il periodo di test. Sono informazioni che nel flusso standard vengono inserite manualmente da parte dell'utente e associate ai record relativi alle prove.
    \item \textbf{Fornitore o richiedente} : costituisce l'entità che è legata alla richiesta dei test oppure che gestisce la fase di esecuzione dei test. Individuare il campione infatti non è sufficiente per distinguere esattamente i record relativi a una prova, potenzialmente lo stesso campione può essere legato a fornitori diversi.
    \item \textbf{Standard di prova e metodi di test} : comprendono i riferimenti normativi (ad esempio norme GB/T o ISO), sono informazioni che vanno a legare il campione ai record delle singole prove. Lo standard, se definito nel documento permette di estrarre un pacchetto di prove necessarie al rispetto dello standard in questione, in alternativa o in aggiunta, sono elencate all'interno di un report le singole prove col relativo metodo di test. Il nome della prova non costituisce tuttavia un legame solido ma richiede una verifica per similarità, cosa non veritiera invece per il metodo dei test, che può essere individuato direttamente sulla base dati.
\end{itemize}